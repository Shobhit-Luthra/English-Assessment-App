...
export default function AppShell({ children, recruiterPreview = false }) {
  const { user, has, signOut } = useAuth()
  const name = user?.profile?.full_name || user?.display_name
  const workspace = has('candidates.view') || recruiterPreview

  const sidebarLinks = []
  if (has('candidates.view') || recruiterPreview) sidebarLinks.push({ to: recruiterPreview ? '/recruiter-preview' : '/dashboard', label: 'Candidates', icon: CANDIDATE_ICON })
  if (has('analytics.view') || recruiterPreview) sidebarLinks.push({ to: recruiterPreview ? '/recruiter-preview/analytics' : '/analytics', label: 'Analytics', icon: ANALYTICS_ICON })
  if (has('roles.manage') && !recruiterPreview) sidebarLinks.push({ to: '/admin', label: 'Admin', icon: ADMIN_ICON })
    ...
    return (
      <div className="h-screen overflow-hidden bg-gray-50 flex">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
          {recruiterPreview && <div className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-2 border-b border-amber-200 bg-amber-50 px-6 py-2 text-sm text-amber-950"><span>Previewing recruiter workspace. You remain signed in as an admin; decisions are recorded under your account.</span><Link to="/admin" className="font-medium text-blue-800 hover:underline">Return to Admin</Link></div>}
